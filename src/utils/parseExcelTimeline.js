...
export const parseExcelTimeline = (workbook, XLSX) => {
    ...
    for (let i = 1; i < epochRows.length; i++) {
        ...
        events.push({
            ...
            isParent: true,
            type: 'epoch',
            order: row[16] != null && row[16] !== '' ? Number(row[16]) : i - 1,
            isMilestone: false,
    ...
    for (let i = 1; i < stageRows.length; i++) {
        ...
        events.push({
            ...
            isParent: false,
            type: 'stage',
            order: row[17] != null && row[17] !== '' ? Number(row[17]) : i - 1,
            isMilestone: false,
    ...
    for (let i = 1; i < eventRows.length; i++) {
        ...
        const evt = {
            ...
            isParent: false,
            type: 'event',
            order: row[18] != null && row[18] !== '' ? Number(row[18]) : i - 1,
            isMilestone: false,
    ...
    for (let i = 1; i < milestoneRows.length; i++) {
        ...
        const evt = {
            ...
            isParent: false,
            type: 'milestone',
            order: row[15] != null && row[15] !== '' ? Number(row[15]) : i - 1,
            isMilestone: true,
